...
aws ec2 describe-spot-instance-requests --filters Name=tag:Name,Values=Frontend Name=state,Values=active --output table | grep InstanceId &>/dev/null
if [ $? -eq 0 ]; then
...
INSTANCE_ID=$(aws ec2 describe-spot-instance-requests --filters Name=tag:Name,Values=Frontend Name=state,Values=active --output table | grep InstanceId | awk '{print$4}')